...
class OpenCVNSA(SelfSupTask):
    def __init__(self, mode=cv2.NORMAL_CLONE):
        self.mode = mode
        ...
        self.width_bounds_pct = self.blender = self.labeller = self.min_obj_pct = self.class_has_foreground \
    ...
    def _collect_continuous_NSA_examples(self, files_to_load):
        ...
        temp_blender = OpenCVPoissonPatchBlender(self.mode,
                                                 (self.blender.norm_args[0],
                                                  self.blender.norm_args[1],
                                                  None))

        for j in files_to_load[1:]:
            new_img, new_img_m = self._load_img_m_pair(j['data_file'])
            new_prop = j['properties'] if 'properties' in j.keys() else load_pickle(j['properties_file'])
            temp_blender.norm_args = (temp_blender.norm_args[0],
                                      temp_blender.norm_args[1],
                                      new_prop['channel_intensity_properties'])

            _, cont_label = patch_ex(new_img, last_img,
                                     shape_maker=self.shape_maker,
                                     patch_transforms=self.transforms,
                                     blender=temp_blender,
    ...
    def calibrate(self, dataset, exp_plans):
        if not self._calibrated:
            ...
            self.blender = OpenCVPoissonPatchBlender(self.mode,
            ...
            print('WARNING: OpenCVNSA has already been calibrated, cannot be done again.')
...
class OpenCVNSAMixed(OpenCVNSA):

    def __init__(self):
        super().__init__(cv2.MIXED_CLONE)
